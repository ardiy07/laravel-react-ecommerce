...
import dataNotifikasi from './DataNotifikasi.json'
import './index.css'
import { getAssetImages } from '../../../utils/pathUtils';

function Notifikasi({ onOpen, onClose }) {
  ...
  return (
    <div className='relative' onMouseEnter={notifikasiOpen} onMouseLeave={notifikasiClose}>
      <span className={`body-icon-notif hover:bg-gray-100 rounded-md ${isOpen ? 'bg-gray-100' : ''}`}>
        ...
        <div className='mt-1 absolute z-10 w-80 max-w-80 bg-white shadow-2xl rounded-md right-0 -mx-32 border'>
          <div className='sticky top-0 bg-white z-10 pt-1 pb-2'>
            <div className='flex justify-between shadow-md px-3 items-center'>
              ...
              <div>
                <a href='#' className='body-icon-setting'>
            ...
            <div className='text-center cursor-pointer'>
              <ul className='grid grid-flow-col w-full text-gray-500'>
                <Links.Page onClick={() => handleClick("Transaksi")} currentPage={currentPage} title="Transaksi" />
                <Links.Page onClick={() => handleClick("Update")} currentPage={currentPage} title="Update" />
              </ul>
            </div>
          </div>
            ...
            {currentPage === "Transaksi" && (
              <div className='pt-1'>
                <div className='flex justify-between px-3'>
                  ...
                  <a href='#'className='text-green-600 tracking-tight font-semibold'>Lihat Semua</a>
                </div>
                <div className='py-1 flex px-1'>
                  <a className='my-auto py-1 px-2 rounded-xl text-sm font-normal hover:bg-slate-200 w-full' href='#'>
                    Menunggu Pembayaran
                  </a>
                </div>
                <div className='flex justify-around px-1 gap-x-2 my-1 border-b-8 pb-7 border-slate-100'>
                  {dataNotifikasi.map((item, index) => (
                    <div key={index} className='items-center justify-center'>
                      <a href={item.url} className='flex flex-col items-center justify-center text-center'>
